...
export default function CompanyCalendarPage() {
    ...
    const getDayTypeStyles = (dayType: string | undefined) => {
        ...
        const styles: Record<string, string> = {
            ...
            "special-working-day": "bg-yellow-100 hover:bg-yellow-200 border-yellow-300",
        };
    ...
    const renderCalendarDays = () => {
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                <div
                    key={day}
                    className={`h-24 p-2 border border-gray-200 cursor-pointer transition-all duration-200 ${getDayTypeStyles(dayData?.day_type)} ${isToday ? "ring-2 ring-blue-500 ring-inset" : ""}`}
                ...
                >
                    <div className="flex justify-between items-start">
                        ...
                        {dayData && (
                            <span className={`text-xs px-2 py-0.5 rounded-full ${getDayTypeBadgeStyles(dayData.day_type)}`}>
                                {dayData.day_type.replace("-", " ")}
                            </span>
                        )}
                    </div>
                    {dayData?.description && (
                        <p className="text-xs text-gray-600 mt-1 line-clamp-2">{dayData.description}</p>
                    )}
                </div>
            );
        }
            ...
            <div className="px-6 py-4 bg-white border-b border-gray-200">
                ...
                <div className="flex items-center gap-4 mt-4">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <span className="text-xs text-gray-600">Special Working Day</span>
                    </div>
                </div>
            </div>
                    ...
                    <div className="space-y-2">
                        <div className="flex items-center justify-between">
                            <span className="font-semibold">
                                {new Date(hoveredDay.calendar_date).toLocaleDateString("en-US", {
                                    weekday: "long",
                                    day: "numeric",
                                    month: "long",
                                    year: "numeric",
                        ...
                        <div className="flex items-center gap-2">
                            <span className={`px-2 py-1 text-xs rounded-full ${getDayTypeBadgeStyles(hoveredDay.day_type)}`}>
