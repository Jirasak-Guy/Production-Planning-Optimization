...
class RLSchedulerInference:
    ...
    def get_obs(self) -> np.ndarray:
        ...
        for i, task in enumerate(self.tasks[:self.MAX_TASKS]):
            ...
            if self.scheduled[i] is not None:
                obs[base + 1] = 1.0
            n_ops = len(self.data.operation_to_work_centers)
            obs[base + 2] = task.operation_id / max(n_ops, 1)
        ...
        for i, (m_id, info) in enumerate(list(self.machines.items())[:self.MAX_MACHINES]):
            ...
            obs[base + 3] = info['workers_required'] / max(self.max_workers, 1)
    ...
    def decode_action(self, action: int, valid_actions: List[Tuple[int, int]]) -> Tuple[int, int]:
        """Map raw model action to closest valid (task_idx, machine_id)"""
        task_a = action // self.MAX_MACHINES
        machine_a = action % self.MAX_MACHINES
        m_list = list(self.machines.keys())

        best = None
        best_dist = float('inf')
        for (t_idx, m_id) in valid_actions:
            m_idx = m_list.index(m_id) if m_id in m_list else 0
            dist = abs(t_idx - task_a) + abs(m_idx - machine_a)
            if dist < best_dist:
                best_dist = dist
                best = (t_idx, m_id)
